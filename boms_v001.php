 ...
 * @Last Modified time: 2026-01-12 23:00:01 
 ...
 * Includeso opzioni per gestire vari tipi di file BOM
...
{	
		...
		if (($handle = fopen($arquivo, "r")) !== FALSE) 
	{
		...
		// Salta la prima riga (intestazione) se richiesto
		if ($parser['skip_header']) {
			fgetcsv($handle, 1000, $parser['delimiter']);
		}
	
		// Esegue l'iterazione delle righe del BOM file per popolare la tabella temporanea
		while (($data = fgetcsv($handle, 1000, $parser['delimiter'])) !== FALSE) 
		{	
			// Verifica che ci siano abbastanza colonne
			$minCols = max($parser['code_col'], $parser['qty_col']) + 1;
			if (count($data) < $minCols) {
				continue; // Salta righe incomplete
			}
			
			// Estrai codice e quantità dalle colonne configurate
			$codice = trim($data[$parser['code_col']]);
			$tmp_quant = trim($data[$parser['qty_col']]);
			
			// Rimuovi apici se richiesto (KiCAD usa "'" attorno ai valori)
			if ($parser['strip_quotes']) {
				$codice = str_replace("'", '', $codice);
				$tmp_quant = str_replace("'", '', $tmp_quant);
			}
			
			// Validazione dati
			if (empty($codice)) {
				continue; // Salta se il codice è vuoto
			}
	...
	if (is_array($resultados) && count($resultados) > 0) {
		echo "<div class='alert alert-info'><i class='fa-solid fa-info-circle me-2'></i>Formato selezionato: <strong>" . htmlspecialchars($parser['description'], ENT_QUOTES, 'UTF-8') . "</strong></div>";
		...
		echo "<thead class='table-light'><tr><th>Codice BOM</th><th>Qty Richiesta</th><th>Qty Magazzino</th><th>Livello Scorte</th><th>Status</th><th>Note/Azioni</th></tr></thead>";
		echo "<tbody>";
		...
		foreach ($resultados as $row) 
		{
			...
			// Quantità magazzino
			echo "<td class='text-center'><span class='badge bg-secondary'>" . htmlspecialchars($row['scorte_corrente'], ENT_QUOTES, 'UTF-8') . "</span></td>";
